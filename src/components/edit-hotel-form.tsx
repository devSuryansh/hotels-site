...
// Image upload component (same as in add hotel form)
...
// String array input component (same as in add hotel form)
...
export function EditHotelForm({
  ...
  // Populate form when hotel prop changes
  useEffect(() => {
    if (hotel && open) {
      form.reset({
        name: hotel.name,
        ...
        priceRange: {
          min: hotel.priceRange[0],
          max: hotel.priceRange[1],
        },
  ...
  const onSubmit = async (data: HotelFormValues) => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`/api/hotels/${hotel.id}`, {
        method: "PUT",
      ...
      if (response.ok) {
        toast({
          ...
          description: "The hotel has been updated with your changes.",
        });
        onHotelUpdated();
        onOpenChange(false);
      } else {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-2xl">
        <SheetHeader>
          ...
          <SheetDescription>
            Update hotel information and manage meta tags.
          </SheetDescription>
        </SheetHeader>
